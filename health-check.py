...
def main():
    ...
    df_ticks = df_ticks.sort_values(['device_uid', 'tick_time'])

    # 2. Calculate Inter-Arrival Times (Jitter)
    # Group by device to ensure we don't diff across different devices
    df_ticks['inter_arrival_ms'] = df_ticks.groupby('device_uid')['tick_time'].diff().dt.total_seconds() * 1000
    ...
    fig = px.scatter_3d(df_features, 
                        ...
                        color_discrete_map={'Failing': 'red', 'At-Risk': 'orange', 'Healthy': 'green'},
                        symbol_map={'Failing': 'cross', 'At-Risk': 'diamond', 'Healthy': 'circle'},
